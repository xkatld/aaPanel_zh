...
def bt_cli(u_input = 0):
    ...
    if not u_input:
        ...
        print("(4) %s                            (11) 开启/关闭 IP + User-Agent 认证"% (public.GetMsg("RELOAD_PANEL")))
        ...
        print("(22) %s                (15) %s"% ("显示面板错误日志",public.GetMsg("CLEAR_SYS_RUBBISH")))
        print("(23) %s       (16) %s"% ("关闭BasicAuth认证","修复面板(检查错误并更新面板文件到最新版本)"))
        print("(24) 关闭Google验证器                     (17) 设置日志切割开启/关闭压缩")
        print("(25) 在面板中修改文件时保存副本           (18) 设置是否自动备份面板")
        # if not os.path.exists('/www/server/panel/data/panel_pro.pl'):
        #     print("                                            (19) Update to aapanel pro version")
        print("(26) 备份到云存储时保留/删除本地备份")
        print("(27) 开启/关闭面板SSL                  (28) 修改面板安全入口")
        print("(0) 取消")
        print(raw_tip)
        try:
    ...
    if u_input == 1:
        ...
        if os.path.exists(pro_path):
            ...
            os.system("curl -k http://io.bt.sy/install/update_pro_en.sh|bash")
        else:
            ...
            os.system("curl -k http://io.bt.sy/install/update_7.x_en.sh|bash")
    elif u_input == 17:
